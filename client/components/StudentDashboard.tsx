...
import { Faculty, BroadcastMessage, DashboardView, FacultyStatus, Appointment, StudentProfile, LeaveRequest, StudentNotification, NotificationRequest } from '../types';
...
import { 
  ...
  CheckCircle2, MinusCircle, XCircle, Calendar, Clock, 
  User, FileText, ExternalLink, GraduationCap, Trash2, 
  Bell, Mail, RefreshCcw, Sun, Moon, List, Calendar as CalendarIcon,
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ facultyList: initialFaculty, broadcasts: initialBroadcasts, onLogout }) => {
  ...
  const availableCount = facultyList.filter(f => f.status === FacultyStatus.AVAILABLE).length;
  const busyCount = facultyList.filter(f => f.status === FacultyStatus.BUSY).length;
  ...
  const handleNotify = (id: string) => {
    ...
    if (!faculty) return;
    const existingReq = storageService.getNotificationRequests().find(r => r.facultyId === id && r.studentId === profile.enrollmentNo);
    if (existingReq) {
      alert("You have already subscribed for updates from this faculty.");
      return;
    }
    const req: NotificationRequest = {
      id: Date.now().toString(),
      studentId: profile.enrollmentNo,
      facultyId: faculty.id,
      facultyName: faculty.name,
      timestamp: Date.now()
    };
    storageService.saveNotificationRequest(req);
    alert(`Success: You will receive an email notification when ${faculty.name} becomes AVAILABLE.`);
  };
  ...
  const handleBookingConfirm = (data: { date: Date; time: string; purpose: string }) => {
    if (bookingFaculty) {
      const newAppointment: Appointment = {
        id: Date.now().toString(),
        facultyId: bookingFaculty.id,
        facultyName: bookingFaculty.name,
        facultyImage: bookingFaculty.image,
        date: data.date,
        time: data.time,
        status: 'Pending',
  ...
  const handleLeaveSubmit = (request: LeaveRequest) => {
    const updatedRequests = [request, ...leaveRequests];
    setLeaveRequests(updatedRequests);
    storageService.saveLeaveRequests(updatedRequests);
  };

  const handleDeleteLeave = (id: string) => {
    if (window.confirm("Withdraw this leave application?")) {
      storageService.deleteLeaveRequest(id);
    }
  };
  ...
  const SidebarItem = ({ view, icon: Icon, label, badge }: { view: DashboardView, icon: any, label: string, badge?: number }) => (
    <button
      ...
      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all mb-1 relative group ${
        currentView === view || (view === DashboardView.FACULTY_LIST && currentView === DashboardView.FACULTY_PROFILE)
      ...
      <div className="w-64 bg-bgSecondary border-r border-border hidden md:flex flex-col p-4">
        ...
        <div className="mb-6 px-2 flex items-center space-x-3 pb-6 border-b border-border">
          <div className="relative group">
            <img src={profile.avatar} alt="Profile" className="w-12 h-12 rounded-2xl border-2 border-border group-hover:border-accent transition-colors" />
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-hide">
          ...
          <SidebarItem view={DashboardView.LEAVE_REQUEST} icon={FileText} label="Leave Requests" />
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="absolute top-4 right-8 z-40 hidden md:flex items-center space-x-4">
           <button 
             ...
             className="p-2.5 bg-card border border-border rounded-2xl hover:bg-bgSecondary text-textSecondary hover:text-accent transition-all shadow-xl flex items-center gap-2 group backdrop-blur-md"
             title="Sync Data"
           >
              ...
              <span className="text-xs font-bold pr-1">Sync</span>
           </button>
        </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 scrollbar-hide">
                      ...
                      <div className="space-y-3">
                         ...
                         {appointments.length === 0 && (
                            <div className="text-center py-10 bg-card/50 border border-border border-dashed rounded-2xl text-textSecondary text-xs">
                               <Calendar className="w-8 h-8 mx-auto mb-2 opacity-10" />
          ...
          {currentView === DashboardView.NOTIFICATIONS && (
             ...
             >
                <div className="flex justify-between items-center px-1">
                   ...
                   <span className="text-[10px] font-black uppercase tracking-widest text-textSecondary bg-bgSecondary px-3 py-1 rounded-full border border-border">Live Activity</span>
                </div>

                <div className="space-y-3">
                  {notifications.length === 0 ? (
                    <motion.div variants={itemVariants} className="text-center py-24 bg-card rounded-3xl border border-border border-dashed">
                        <Bell className="w-16 h-16 text-textSecondary mx-auto mb-4 opacity-10" />
                        <h3 className="text-lg font-bold text-textPrimary">All caught up!</h3>
                        <p className="text-textSecondary text-sm">You have no new notifications in your inbox.</p>
                     </motion.div>
                  ) : (
                    notifications.map(notif => (
                       <motion.div key={notif.id} variants={itemVariants} className={`bg-card border rounded-2xl p-5 flex items-start space-x-4 transition-all hover:scale-[1.01] ${
                          notif.isRead ? 'border-border opacity-70' : 'border-accent ring-1 ring-accent/20 shadow-xl shadow-accent/5'
                       }`}>
                          <div className="flex-shrink-0 mt-1">
                             {notif.facultyImage ? (
                                <img src={notif.facultyImage} className="w-11 h-11 rounded-2xl border-2 border-border" alt="Sender" />
                             ) : (
                                <div className="w-11 h-11 rounded-2xl bg-bgSecondary flex items-center justify-center border-2 border-border">
                          ...
                          <div className="flex-1 min-w-0">
                             <div className="flex justify-between items-start mb-1">
                                <h4 className={`text-base leading-tight ${notif.isRead ? 'font-bold text-textPrimary' : 'font-black text-white'}`}>
                                   {notif.title}
                                </h4>
                                <span className="text-[10px] text-textSecondary whitespace-nowrap ml-2 font-mono">
                             ...
                             {!notif.isRead && (
                                <span className="inline-block mt-3 text-[10px] font-black bg-accent text-bgPrimary px-3 py-0.5 rounded-full uppercase">
                                   Urgent Update
                                </span>
                             )}
                          </div>
                  ...
                  <div className="flex items-center space-x-2 overflow-x-auto pb-1 md:pb-0 scrollbar-hide">
                    <button
                      ...
                      className={`px-5 py-2.5 rounded-2xl text-xs font-bold whitespace-nowrap transition-all border ${
                        selectedDept === 'All' ? 'bg-accent text-bgPrimary border-accent shadow-lg shadow-accent/20' : 'bg-bgPrimary text-textSecondary border-border hover:border-accent'
                      }`}
                    >
                      All Depts
                    </button>
                    {DEPARTMENTS.map(dept => (
                      <button
                        ...
                        className={`px-5 py-2.5 rounded-2xl text-xs font-bold whitespace-nowrap transition-all border ${
                          selectedDept === dept ? 'bg-accent text-bgPrimary border-accent shadow-lg shadow-accent/20' : 'bg-bgPrimary text-textSecondary border-border hover:border-accent'
                        }`}
                      >
                        {dept}
                      </button>
          ...
          {currentView === DashboardView.LEAVE_REQUEST && (
            <motion.div 
              variants={containerVariants}
              initial="hidden"
              whileInView="visible"
            ...
            >
              <div className="flex justify-between items-center px-1">
                <h2 className="text-2xl font-black text-textPrimary">Leave Portal</h2>
                <button 
                  onClick={() => setShowLeaveModal(true)}
                  className="px-6 py-3 bg-accent text-bgPrimary rounded-2xl font-bold hover:bg-accentHover transition-all flex items-center shadow-lg shadow-accent/20"
                >
              ...
              <div className="space-y-4">
                {leaveRequests.length === 0 ? (
                  <motion.div variants={itemVariants} className="text-center py-20 bg-card rounded-3xl border border-border border-dashed">
                    <FileText className="w-16 h-16 text-textSecondary mx-auto mb-4 opacity-10" />
                    <p className="text-textSecondary text-sm">You haven't submitted any leave applications yet.</p>
                  </motion.div>
                ) : (
                  leaveRequests.map(req => (
                    <motion.div key={req.id} variants={itemVariants} className="bg-card border border-border rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 group hover:border-accent transition-all shadow-sm">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <span className="text-[10px] font-black text-accent uppercase tracking-widest bg-accent/10 px-3 py-1 rounded-full border border-accent/20">{req.type}</span>
                          <span className={`text-[10px] font-black uppercase px-3 py-1 rounded-full border ${
                            req.status === 'Pending' ? 'text-warning border-warning/30 bg-warning/10' :
                            req.status === 'Approved' ? 'text-emerald-500 border-emerald-500/30 bg-emerald-500/10' :
                            'text-error border-error/30 bg-error/10'
                          }`}>
                            {req.status}
                          </span>
                        </div>
                        <p className="text-textPrimary font-bold text-xl mb-1">{req.reason}</p>
                      ...
                      <div className="flex flex-col items-end gap-3">
                        <span className="text-[10px] text-textSecondary font-mono uppercase bg-bgSecondary px-2 py-1 rounded-md border border-border">
                           Submitted {new Date(req.timestamp).toLocaleDateString()}
                        </span>
                        {req.status === 'Pending' && (
                          <button 
                            onClick={() => handleDeleteLeave(req.id)}
                            className="text-xs text-error hover:underline flex items-center font-bold"
                          >
                              ...
                              <>
                                ...
                                <div className="space-y-1">
                                  {dayApts.map(apt => (
                                    <div 
                                      key={apt.id} 
                                      className={`text-[9px] font-bold p-1 rounded border leading-none truncate ${
                                        apt.status === 'Pending' ? 'bg-warning/10 border-warning/30 text-warning' : 'bg-accent/10 border-accent/30 text-accent'
                                      }`}
                    ...
                    {appointments.length === 0 ? (
                       ...
                       appointments.map(apt => (
                         <motion.div key={apt.id} variants={itemVariants} className="bg-card border border-border rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:border-accent transition-all shadow-sm">
                            <div className="flex items-center space-x-4">
                               <img src={apt.facultyImage} alt={apt.facultyName} className="w-14 h-14 rounded-2xl object-cover border-2 border-border" />
                               <div>
                            ...
                            <div className="flex items-center justify-between w-full md:w-auto gap-8">
                               ...
                               <div className="flex flex-col items-end gap-3">
                                  <div className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                                      apt.status === 'Pending' ? 'text-warning bg-warning/10 border-warning/30' : 
